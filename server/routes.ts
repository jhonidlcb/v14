...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // Generate PDF content for the stage invoice
      const invoiceNumber = `INV-STAGE-${stage[0].projectId}-${stage[0].id}`;
      const paidDate = stage[0].paidAt ? new Date(stage[0].paidAt).toLocaleDateString('es-ES') : 'N/A';
      
      const pdfContent = `FACTURA DE ETAPA DE PAGO - SOFTWAREPAR
...
Etapa: ${stage[0].stageName}
Porcentaje: ${stage[0].stagePercentage}%
Monto Pagado: $${parseFloat(stage[0].amount).toFixed(2)} USD

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

INFORMACIÓN DE PAGO:
Método de Pago: ${stage[0].paymentMethod || 'N/A'}
Estado: PAGADO ✓
Fecha de Pago: ${paidDate}
...
- Esta factura corresponde al pago de la etapa "${stage[0].stageName}" del proyecto "${project.name}"
- El pago fue procesado y verificado por nuestro equipo administrativo
- Para consultas sobre esta factura, contactar al email de soporte

═══════════════════════════════════════════════════════════════

Gracias por confiar en SoftwarePar para el desarrollo de su proyecto.

Documento generado el: ${new Date().toLocaleString('es-ES', { timeZone: 'America/Asuncion' })}`;
